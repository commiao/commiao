...
public class GeneratorProperties {
	...
	private static final Logger logger = LoggerFactory.getLogger(GeneratorProperties.class); 
	...
	public static void reload() {
		try {
		    logger.info("------------------------第一步加载配置文件开始--------------------------------------");
		    logger.info("从classpath开始加载 GeneratorPropeties:"+Arrays.toString(PROPERTIES_FILE_NAMES));
			Properties p = new Properties();
			String[] loadedFiles = PropertiesHelper.loadAllPropertiesFromClassLoader(p,PROPERTIES_FILE_NAMES);
			logger.info("GeneratorPropeties 加载成功,文件以及路径:"+Arrays.toString(loadedFiles));
			
			setSepicalProperties(p, loadedFiles);
			
			setProperties(p);
			
			logger.info("------------------------第一步加载配置文件结束--------------------------------------");
		}catch(IOException e) {
	...
	private static void setSepicalProperties(Properties p, String[] loadedFiles) {
		if(loadedFiles != null && loadedFiles.length > 0) {
			String basedir = p.getProperty(GeneratorConstants.BASE_DIR.code);
			if(basedir != null && basedir.startsWith(".")) {
				p.setProperty(GeneratorConstants.BASE_DIR.code, new File(new File(loadedFiles[0]).getParent(),basedir).getAbsolutePath());
			}
		}
	}
...
//            GLogger.debug("[Property] "+entry.getKey()+"="+entry.getValue());
            logger.debug("通过配置文件设置key=value: "+entry.getKey()+"="+entry.getValue());
        }
	}
