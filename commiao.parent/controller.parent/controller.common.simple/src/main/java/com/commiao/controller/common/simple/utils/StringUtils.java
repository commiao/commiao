package com.commiao.controller.common.simple.utils;
...
public class StringUtils {
	...
	public static String[] splitToArray(String str, String delim) {
		if(delim==null || delim.equals("")) delim=",";
	 ...
	 * @param str
	 * @param length
	 * @param removeTags 是否去掉空白字符和特殊格式
	 * @param ellipsis 是否自动加...
	 * @return
	 */
	public static String breakString(String str, int length, boolean removeTags, boolean ellipsis) {
		String s = str;
		if(removeTags){
			s = s.replaceAll("\\s*|\t|\r|\n",""); 
		}
	...
	public static String date2chinese(Date date,int format){
		...
		String[] NUMBERS = { "０", "一", "二", "三", "四", "五", "六", "七", "八", "九" ,"十"};
		...
		if(day<10){
			...
			chinese.append(NUMBERS[2]).append(NUMBERS[10]).append(NUMBERS[day%20]);
		}else if(day==30){
			chinese.append(NUMBERS[3]).append(NUMBERS[10]);
		}else{
			chinese.append(NUMBERS[3]).append(NUMBERS[10]).append(NUMBERS[day%30]);
		}
		chinese.append("日");
		return chinese.toString();
	}
	...
	public static String number2chinese(String number){
    	...
    	String[] IUNIT = { "点", "十", "百", "千", "万", "十", "百", "千", "亿", "十", "百", "千", "万", "十", "百", "千" };
		...
		for (int i = 0; i < length; i++) {
			...
			chinese.append(array[i] == 0 ? key : ( NUMBERS[array[i]] + ((length-i)==1?"":IUNIT[length - i - 1])));
		}
	...
	public static String number2money(String number){
		...
		for (int i = 0; i < length; i++) {
			...
			chinese.append(array[i] == 0 ? key : (NUMBERS[array[i]] + IUNIT[length - i - 1]));
		}
		...
		if(decimalStr != null && !decimalStr.equals("")){
			...
			for (int i = 0; i < array.length; i++) {
				...
				if (i == 3)
					break;
				chinese.append(array[i] == 0 ? "" : (NUMBERS[array[i]] + DUNIT[i]));
			}
		}
     ...
     * 删除字符串中的html格式  
     * @param input 被转换的带有html的文本
     * @param length 截取字符的长度
     * @param ellipsis 是否自动加入省略号  
     * @return  
     */  
    public static String removeHTMLTags(String input, int length, boolean ellipsis) {   
	 ...
	 * 将字符串由下划线（'_'）改为驼峰式
	 * @param s
	 * @param capitalizeUpper 首字母是否大写
	 * @return
	 */
	public static String toCamelCase(String s, boolean capitalizeUpper) {
		return toCamelCase(s, capitalizeUpper, '_');
	 ...
	 * @param s 字符串
	 * @param capitalizeUpper 首字母是否大写
	 * @param SEPARATOR 字符串中的特殊符号
	 * @return
	 */
	public static String toCamelCase(String s, boolean capitalizeUpper, char SEPARATOR) {
	...
	public static long countWords(String param) {
		...
		if(param != null && !param.equals("")){
			String str=removeHTMLTags(param.toString());//TODO 去掉HTML
...
//		//测试数字转成中文
//		System.out.println("012300000 is " + StringUtils.number2chinese("012300000"));
//		System.out.println("012300001 is " + StringUtils.number2chinese("012300001"));
//		System.out.println("012.300000 is " + StringUtils.number2chinese("012.300000"));
//		System.out.println("012300.001 is " + StringUtils.number2chinese("012300.001"));
//		//测试生成钱
//		String number = "1.23";
...
//		System.out.println(str + " " + StringUtils.removeHTMLTags(str,2,true));
//		
//		
...
//		System.out.println(end - start);
		...
		System.out.println(StringUtils.date2chinese(DateTimeUtils.parse("2000-10-11","yyyy-MM-dd") ,3));
	}
