package com.framework.common.core.domain.model.map;
...
public class ParamterMap{
	...
	private static java.util.Map<String,Object> getInstance(){
		...
		param.put("status", CommonConstants.GLOBLE_YES);
		return param;
	}
	...
	public static java.util.Map<String,Object> build(java.util.Map<String,Object> paramterMap){
		...
		paramterMap.put("status", CommonConstants.GLOBLE_YES);
		return paramterMap;
	}
	...
	public static java.util.Map<String, Object> build(Object obj) {
		...
		StringBuffer sb = new StringBuffer("sql查询条件：");
		try {
			...
			for (int i = 0; i < descriptors.length; i++) {
				...
				if (!org.apache.commons.lang.StringUtils.equals(name, "class")) {
					if(propertyUtilsBean.getNestedProperty(obj, name)!=null){
						...
						sb.append("【").append(name).append("=").append(propertyUtilsBean.getNestedProperty(obj, name)).append("】");
					}
				}
			}
		} catch (Exception e) {
	...
	public static java.util.Map<String,Object> build(String keys,Object... objs){
		...
		StringBuffer sb = new StringBuffer("##sql#val：");
		for(int i=0;i<objs.length;i++){
			param.put(keyMap.get(i), objs[i]);
			sb.append("【").append(keyMap.get(i)).append("=").append(objs[i]).append("】");
		}
		System.out.println(sb.toString());
		return param;
	}
	...
	private static java.util.Map<Integer,String> getParamKeys(String keys){
		...
		keys = keys.trim();
		String[] s = keys.split(" ");
		StringBuffer sb = new StringBuffer("##sql#key：{ ");
